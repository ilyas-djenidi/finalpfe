...
def update_last_login(user_id: int) -> None:
    try:
        db   = _get_db()
        resp = db.table("users").select("login_count").eq("id", user_id).execute()
        count = int((resp.data[0].get("login_count", 0) if resp.data else 0)) + 1
...
def delete_report(token: str) -> tuple[bool, str]:
    try:
        db   = _get_db()
        resp = db.table("scan_reports").delete().eq("token", token).execute()
        if not resp.data:
