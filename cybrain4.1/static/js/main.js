...
(function (config, ui, form, particles) {
    ...
    function _initStats() {
        const { STATS, TIMINGS } = config;

        setTimeout(() => {
            ui.animateCount(document.getElementById('statScans'),   STATS.SCANS);
            ui.animateCount(document.getElementById('statThreats'), STATS.THREATS);
        }, TIMINGS.STATS_DELAY);
    }
