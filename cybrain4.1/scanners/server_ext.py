...
# المصدر: NVD / Apache Security Reports
...
def _check_sensitive_paths(base_url: str, session: requests.Session) -> list[Finding]:
    """يتحقق من وجود مسارات حساسة مكشوفة."""
    findings = []
    ...
    for item in SENSITIVE_PATHS:
        ...
        try:
            resp = session.get(
                ...
                verify=False,    # نريد الاستجابة حتى لو الـ cert غير صالح
            )

            # 200 أو 403 (موجود لكن محمي) يعني المسار موجود
            if resp.status_code == 200:
                ...
                # موجود لكن محمي — ملاحظة فقط
                findings.append(Finding(
                    check=f"protected_path_{item['path'].strip('/').replace('/', '_') or 'root'}",
                    severity=Severity.INFO,
                    title=f"مسار موجود لكن محمي: {item['path']}",
                    description=f"الـ path موجود (403) لكن محمي. تحقق أن هذا مقصود.",
                    evidence=f"HTTP 403 - {url}",
                    remediation="",
                ))

        except requests.RequestException:
            # timeout أو connection error — نتجاهل هذا المسار
            pass
...
def _check_apache_version(version: str) -> list[Finding]:
    """يتحقق من ثغرات الإصدار المعروفة."""
    findings = []
    ...
    if vuln:
        findings.append(Finding(
            ...
            remediation="حدّث Apache إلى أحدث إصدار مستقر.",
            cves=vuln["cves"],
        ))
    else:
        # إصدار غير مُدرج كخطير — لكن نُنبّه إذا قديم جداً
        try:
            ...
            if major == 2 and minor < 4:
                findings.append(Finding(
                    ...
                    evidence=f"Apache version: {version}",
                    remediation="رقّ إلى Apache 2.4.x أحدث إصدار.",
                ))
        except ValueError:
            pass
