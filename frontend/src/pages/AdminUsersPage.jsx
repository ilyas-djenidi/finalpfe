...
const AdminUsersPage = () => {
    ...
    const handleDelete = async (uid, targetUsername) => {
        if (!window.confirm(`Permanently delete account "${targetUsername}"? This cannot be undone.`)) return;
        try {
    ...
    const handleToggleActive = async (uid, currentlyActive) => {
        try {
            await axios.patch(`/api/admin/users/${uid}`, {
            ...
            <div className="mb-8">
                ...
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm max-w-2xl">
                        Provision, modify, and revoke access for platform operators.
                    </p>
                </div>
            </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                        ...
                        <form onSubmit={handleCreateUser} className="space-y-4">
                            ...
                            <p className="text-[10px] text-slate-400 leading-relaxed">
                                Password must be ≥10 chars with uppercase, lowercase, digit, and special character.
                            </p>
                            <button
                                type="submit"
                ...
                <div className="lg:col-span-8">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/20">
                            ...
                            <span className="text-xs text-slate-500">{usersList.length} account{usersList.length !== 1 ? 's' : ''}</span>
                        </div>

                        {fetchError && (
                            <div className="p-6 text-red-500 text-sm flex items-center gap-2">
                                            ...
                                            <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {u.username === user.username && (
                                                            <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-primary-100 text-primary-700 dark:bg-primary-500/10 dark:text-primary-400 border border-primary-200 dark:border-primary-500/20">
                                                                YOU
                                                            </span>
                                                        )}
                                                    </div>
                                                ...
                                                <td className="px-6 py-4">
                                                    <button
                                                        ...
                                                        className="flex items-center gap-1.5 text-xs font-medium transition-colors disabled:opacity-40 disabled:cursor-default"
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center justify-end gap-1">
                                                        <Link
                                                            ...
                                                            className="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-500/10 dark:hover:text-primary-400 rounded-lg transition-colors"
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={u.username === user.username}
                                                            className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 dark:hover:text-red-400 rounded-lg transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:cursor-default"
                                                            title="Revoke Access"
                                                        >
